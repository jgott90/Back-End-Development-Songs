...
def create_song():
    ...
    song_input = request.json

    print(song_input["id"])

    # if the id is already there, return 303 with the URL for the resource
    song = db.songs.find_one({"id": song_input["id"]})
    if song:
        return {
            "Message": f"song with id {song_input['id']} already present"
        }, 302

    song_insert: InsertOneResult = db.songs.insert_one(song_input)

    return {"inserted id": parse_json(song_insert.inserted_id)}, 201 
